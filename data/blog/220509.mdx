...
- 각 input 에서 onChange 옵션으로 clearErrors(“result”)를 해준다. (submit 에러가 발생 이후 해당 error 를 지우지 못하는 상태를 방지함 - input 에 onChange event 발생시 자동으로 에러 삭제)
...
- const location = useLocation(); const state:{username} = location;

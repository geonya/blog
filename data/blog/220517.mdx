...
- Container flex: 1 (Native 에서만 허용되는 것으로 FlexBox 에서 해당 비율만큼 공간을 차지함)
...
- styled-components/native 에서는 부모 CSS 속성이 자식에게 상속되지 않음
- onPress type 을 정의하는 방법
...
- `TextInput` 컴포넌트를 이용하여 회원가입 폼란 만들기
- `returnKeyType` 등을 이용하여 키보드 구성 바꾸기
- `secureTextEntry` 비밀번호 부분에는 이 옵션을 꼭 넣어줘야함
- Web 에서는 Image 요소가 안나오는데 문제 해결 : max-widght 나 max-height 를 쓰지 말고 width/height 로 정확하게 잡아줘야함
...
- `KeyboardAvoidingView` 컴포넌트는 키보드에 래핑된 요소들이 가려지는걸 방지함 behavior 옵션으로 padding 을 누르면 다른 요소도 함께 움직일 수 있음
...
- OS가 web 일 경우 `TouchableWithoutFeedback` 기능을 막음
...
- `npm i react-hook-form` 으로 패키지 설치하기
- react-hook-form 은 웹과 같은 방식으로 네이티브에서 사용할 수 있다.

<br />

# NewsPresso HBR Vol.1.3 Reading
